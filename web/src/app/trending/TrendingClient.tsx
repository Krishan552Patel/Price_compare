...
export default function TrendingClient({ initialCards }: Props) {
  ...
  // Data — pre-populated from SSR so first paint shows cards immediately
  const [cards, setCards]               = useState<TrendingCard[]>(initialCards);
  const [previousCards, setPreviousCards] = useState<TrendingCard[]>(initialCards);
  const [loading, setLoading]           = useState(false); // false: SSR data ready
  const [filterOptions, setFilterOptions] = useState<FilterOptions | null>(null);

  // Skip the very first effect run — initialCards already has the default data.
  // After that, every filter change goes through the API.
  const hasMounted = useRef(false);

  // Load filter options once (for the "Card Filters" panel)
  useEffect(() => {
  ...
  // Fetch trending — only fires when debounced number values settle.
  // Optimistic UI: keep showing current cards (dimmed) while the request is in flight
  // instead of blanking them with a skeleton.
  ...
  // Show stale cards dimmed while loading new ones (optimistic UI).
  // Only fall back to skeletons if there's genuinely nothing to show yet.
  const displayCards = loading && previousCards.length > 0 ? previousCards : cards;
  const upCount   = displayCards.filter((c) => c.price_change > 0).length;
  const downCount = displayCards.filter((c) => c.price_change < 0).length;
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {displayCards.length > 0 && (
        <div className="flex items-center gap-4 mb-4 text-sm">
          <span className="text-gray-400">{displayCards.length} cards found</span>
          {upCount > 0 && (
          ...
          {/* Subtle spinner while refreshing — no full skeleton flash */}
          {loading && (
      ...
      {loading && displayCards.length === 0 ? (
        ...
        // Dim the previous results slightly while the next fetch is in flight
        <div
          className={cx(
        ...
        >
          {displayCards.map((card) => (
            <TrendingRow key={card.printing_unique_id} card={card} />
          ))}
        </div>
      )}
    </div>
