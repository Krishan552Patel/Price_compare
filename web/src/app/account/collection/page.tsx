...
interface CollectionRow {
  ...
  acquired_price: number | null; notes: string | null; hidden: boolean;
}
        ...
        <div className="space-y-2">
          {visibleRows.map(row => {
            ...
            return (
              <div
                key={row.id}
                className={`bg-gray-900 border rounded-xl overflow-hidden transition ${row.hidden ? "border-gray-800/40 opacity-60 hover:opacity-90" : "border-gray-800 hover:border-gray-700"}`}
              >
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    ...
                    <button
                      ...
                      title={row.hidden ? "Hidden from friends — click to show" : "Visible to friends — click to hide"}
                      className={`transition p-1 ${row.hidden ? "text-yellow-500 hover:text-yellow-300" : "text-gray-600 hover:text-gray-400"}`}
                    >
                      {row.hidden ? (
